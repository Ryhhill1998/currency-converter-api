...
class HttpEcbClient(EcbClient):
    def __init__(self, client: httpx.Client, url: str, data_format: str, observations: int) -> None:
        self.client = client
        self.url = url
    ...
    def fetch_latest_rates(self) -> bytes:
        response = self.client.get(url=self.url, params=self._params, follow_redirects=True)
